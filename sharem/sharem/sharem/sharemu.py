...
foundDLLAddresses64 = os.path.join(os.path.dirname(__file__), "foundDLLAddresses64.txt")
...
def loadDlls(mu):
    ...
    export_dict = iter_and_dump_dlls(mu, em, export_dict, source_path, save_path, mods)
...
def hook_WindowsAPI(uc, addr, ret, size, funcAddress):
    ...
    try:
        funcName = export_dict[hex(funcAddress)][0]
        dll = export_dict[hex(funcAddress)][1]
        dll = dll[0:-4]
    ...
    try:
        ...
        try:
            ...
            hook_default(uc, eip, esp, funcAddress, export_dict[hex(funcAddress)][0], addr)
