...
def hook_code(uc, address, size, user_data):
    ...
    if address == 0x1200000b:
        lul = uc.mem_read(uc.reg_read(UC_X86_REG_ESP), 4)
        lul = unpack('<i', lul)[0]
        print("THIS IS OUR RETURN LOC: ", hex(lul))
...
def test_i386(mode, code):
    ...
    # Code coverage test 3 - Proper exit?
...
def startEmu(arch, data, vb):
    ...
    while True:
        print("cov len: ", len(coverage_objects))
        if em.arch == 32:
