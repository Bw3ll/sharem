...
#####################
	...
	def artifactsOut(self,logged_dlls):
		text_output = ""
		var = Variables()
		art = var.art
		...
		if emulation_multiline:
			...
			emu_pathMove_list = self.multiLineTuple(art.path_move,' -> ')
			emu_pathCopy_list = self.multiLineTuple(art.path_copy,' -> ')
			
			emu_filesMisc_list = self.multiLineTuple(art.file_artifacts,' : ')
			...
			emu_filesCopy_list = self.multiLineTuple(art.files_copy, ' -> ')
			emu_filesMoved_list = self.multiLineTuple(art.files_move, ' -> ')
			emu_filesHashes_list = self.multiLineTuple(art.files_hashes,' : ')
			
			emu_commandline_list = self.multiLine(art.commandLine_artifacts)
			
			emu_webArtifacts_list = self.multilineWeb(art.web_artifacts)
			...
			emu_registry_edit_list = self.multiLineTuple(art.registry_edit_keys,'\t\n')
			emu_registry_delete_list = self.multiLineTuple(art.registry_delete_keys,'\t\n')
			...
			emu_commandline_list = ", ".join(art.commandLine_artifacts)
...
#####################
	...
	def multiLineTuple(self,artifact,string:str):
		emu_artifact_list = ''
		if(len(artifact) > 0):
	...
	def multilineWeb(self,artifact):
		emu_artifact_list = ''
		if(len(artifact) > 0):
				for each in artifact:
					if (type(each) == tuple):
...
############################			Documentation			############################
############################################################################################
#	Generates the text and screen output for the main program. 