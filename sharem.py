...
def work_from_directory():
	...
	# print("$$$$$$$$$$$$$$$", list_of_pe64, len(list_of_pe64))
...
def checkForValidAddress2(val_a,val_b1, val_b2, sizeShell, off_PossibleBad,data, num_bytes):
	...
	try:
		if str(val_b2) in off_PossibleBad:
			...
			# val_b=res
			addy=int(val_a,16)
			modifysByRange(data, addy,addy+num_bytes,"d")
			# val_b =  val_b+ " (??)"
...
def createDisassemblyLists():
	...
	for cAddress in tempAddresses:
		if mode=="ascii":
			...
			startHex=int(cAddress,16)
			try:
				endHex=int(tempAddresses[j+1],16)
			except:
				endHex=len(m[o].rawData2)
			# print ("val!: ", hex(startHex), hex(endHex), hex(len(m[o].rawData2)))
			# print(binaryToStr(m[o].rawData2[startHex:endHex]))
			out='{:<10s} {:<35s} {:<26s}{:<10s}\n'.format(cAddress, tempMnemonic[j] + " " + tempOp_str[j], binaryToStr(m[o].rawData2[startHex:endHex]), toString(m[o].rawData2[startHex:endHex]))
		else:
			# val =  hex(int(i.address)) + ":\t" + i.mnemonic + " " + i.op_str+"\n"
			out=('{:<10s} {:<35s}\n'.format(((cAddress)), tempMnemonic[j] + " " + tempOp_str[j]))


		if sBy.comments[int(cAddress,16)] !="":
			# print ("gotttts one")
			# val_b=sBy.comments[int(hex(int(i.address)),16)]
			# val_b=sBy.comments[int(hex(int(pAddress)),16)]
			val_b2=sBy.comments[int(cAddress,16)]
			val_comment =('{:<10s} {:<35s} {:<26s}{:<10s}\n'.format(nada, val_b2, nada, nada))
			out+=val_comment		
		truth,res=checkForLabel( cAddress,labels)
		if truth:
			out=res+out
...
def checkForBad00(data, offset, end):
	...
	if len(ans)>0:
		...
		modifysByRange(data, ans[0], end,  "d")
	...
	while offset < maxSize:
		...
		if w==(length):
			if dbFlag==True:
				...
				curDisassembly =('{:<10s} {:<35s}{:<26s}{:<10s}\n'.format(startAddString, stringVala,dbOut,nada ))
				stringVal+=curDisassembly
				addDis(startAddString,curDisassembly, "string", "","E")
				# stringVal= beforeS + stringVal
				dbOut=""
				dbFlag=False
				if len(dbOut)>0:
...
def anaFindAlign2(data):
	...
	# for datum in data:
		...
		if total > 3:
			...
			if total > 6:
				modifysByRange(data, offset+3, offset+distance+total, "d")
	...
	for x in dataRangeStart:
		...
		try:
			...
			if str(hex(dataRangeEnd[t-1])) not in labels:
				...
				if distance <=5:
					dprint2 ("make data?")
					modifysByRange(data, dataRangeEnd[t-1],dataRangeEnd[t-1]+distance, "d")
			else: 
...
def disHereAnalysisOLD(data,offset, end, mode, CheckingForDB): #
	global labels
	global offsets
	global off_Label
	global off_PossibleBad
	...
	for i in callCS.disasm(CODED2, address):
		...
		while (num_bytes>0):	
			if num_bytes>0:
				...
				dprint2(type(val_c))
				val2.append(val)
				val3.append(str(hex(int(i.address))))
				val5.append(val)
				CantSkip=False
		if CantSkip:
			val2.append(val)
...
def modifysByRange(data, start,end, dataType):
	...
	for x in sBy.bytesType:
		if (t>=start) and (t < end):
			dprint2 ("before", sBy.bytesType[t])
			sBy.bytesType[t]=BytesBool
			
			# dprint2("changing value @ " + str(hex(t)))
			dprint2 (sBy.bytesType[t], " value: ", hex(sBy.values[t]))
			if BytesBool:
...
def takeBytes(shellBytes,startingAddress):
	dprint2 ("takeBytes:", hex(startingAddress))
	global sBy
	global FindStringsStatus
	global shellEntry
	# print ("take bytes")
	...
	for x in shellBytes:
		...
		i+=1
	# print ("shellBytes", len(sBy.comments))
	#rhere
	...
	# print ("sizeOfOut:", len(out))
	dprint ("out:", out)
	# dp(out2)
	return out,assembly


def takeBytesOLDJan(shellBytes,startingAddress):
	dprint2 ("takeBytes:", hex(startingAddress))
	global sBy
	global FindStringsStatus
	# print ("take bytes")
	...
	out=findRange(shellBytes, startingAddress,len(sBy.offsets)-1)  #1st time helps do corrections
	# addEntryPoint()
	# dp(out+str(hex(len(m[o].rawData2))))
...
def addComments():
	...
	for item in m[o].save_PEB_info:
		...
		ldr=item[6]
		sBy.comments[int(ldr,16)] = "; load PEB_LDR_DATA LoaderData"
		mods=item[7]
		modAdd=mods[0]
		modText=mods[1]
		# sBy.comments[int(mods,16)] = "; LIST_ENTRY InMemoryOrderModuleList"
		sBy.comments[int(modAdd,16)] = "; "+ modText
	...
	for item in m[o].save_PushRet_info:
		...
		pushReg=push[1]
		retOffset=item[6]
		sBy.comments[int(pushOffset,0)] = "; pushing return address "
		sBy.comments[int(retOffset,0)] = "; returning to " + pushReg
	...
	for item in m[o].save_Callpop_info:
		...
		sBy.comments[int(pop_offset,16)] = " ; GetPC"

	for item in m[o].save_FSTENV_info:
		FPU_offset = item[5]
		FSTENV_offset = item[6]
		sBy.comments[int(FPU_offset,16)] = " ; floating point to set up GetPC"
		sBy.comments[int(FSTENV_offset,16)] = " ; GetPC"

	for item in m[o].save_Heaven_info:
		heaven_offset = item[5]
		pushOffset=item[6]
		destLocation=item[7]
		sBy.comments[int(heaven_offset,16)]= " ; invoking Heaven's Gate technique"
		try:
			sBy.comments[(pushOffset)]= " ; Heaven's gate destination address: " + str(destLocation)
		except:
			sBy.comments[int(pushOffset,16)]= " ; Heaven's gate destination address: " + str(destLocation)
	for item in m[o].save_Egg_info:
		eax = item[5]
		c0_offset = item[6]
		
		sBy.comments[int(c0_offset	,16)] = " ; Calling Windows syscall - value: " + eax 

	cur=sBy.comments[shellEntry]
	sBy.comments[shellEntry] = cur + " ; ***Shellcode Entry Point, offset " + str(hex(shellEntry)) +"***"
...
def findRangeOldJan(data, startingAddress, end2):
	global bit32
	global FindStringsStatus
	global shellEntry
	if bit32:
	...
	analysis= disHereAnalysis(data, startingAddress, end, "ascii", True)
...
def anaFindStrings(data, startingAddress):
	...
	for word, offset, wordLength,instructionsLength in pushStringsTemp:
		...
		if goodString(data,word,4):
			...
			modifysByRange(data, offset-2, offset+distance, "i") # -2 is a correction
	...
	while current < max:
		...
		if not typeBytes:
			...
			if sBy.strings[start]==True:
				...
				dprint2 (hex(start))
				dprint2 ("AFS: strings ", hex(xStart), hex(ydis), sBy.stringsValue[start])
		start, current, distance, typeBytes = findRange2(current)
		##reset once done - do analysis again to catch any changes 
		if (current) == max and reset != True:
			reset=True
...
def shellDisassemblyInit(shellArg, silent=None):
	...
	txtDis.write(disassembly+assemblyBytes)
...
def discoverUnicodeStrings(max_len=None):
	...
	if max_len==None:
		max_len=42
	bWideStringFound = False
	print("\n"+yel + " Searching for unicode strings...", end="")
	curLen = len("Searching for unicode strings")
...
def discoverAsciiStrings(max_len=None):
	...
	# print("o ----> ", o)
	curLen = len("Searching for Ascii strings")
	print(yel + " Searching for Ascii strings..."+res, end="")
	if rawHex:
...
def discoverStackStrings(max_len=None):
	...
	if max_len==None:
		max_len=42
	print(yel + " Searching for push stack strings...", end="")
